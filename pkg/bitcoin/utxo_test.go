...
func TestUTXO_Creation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			txHash, err := NewHash256FromString(tt.txHash)
			if err != nil {
				t.Fatalf("Failed to parse transaction hash: %v", err)
			}

			utxo := NewUTXO(txHash, tt.outputIndex, tt.amount, tt.scriptPubKey)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, op := range tt.operations {
				txHash, err := NewHash256FromString(tt.txHashes[i])
				if err != nil {
					t.Fatalf("Failed to parse transaction hash: %v", err)
				}

				if op == "add" {
					utxo := NewUTXO(txHash, tt.outputIndex[i], tt.amounts[i], []byte{0x76, 0xa9})
					utxoSet.Add(utxo)
				} else if op == "remove" {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.setupUTXOs && i < len(testHashes); i++ {
				txHash, _ := NewHash256FromString(testHashes[i])
				utxo := NewUTXO(txHash, uint32(i), testAmounts[i], []byte{0x76, 0xa9})
				utxoSet.Add(utxo)
			}

			// Search for UTXO
			searchHash, err := NewHash256FromString(tt.searchTxHash)
			if err != nil {
...
func TestUTXOSet_Validation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Validate the spend
			inputHash, _ := NewHash256FromString(tt.inputTxHash)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, amount := range tt.utxoAmounts {
				...
				txHash, _ := NewHash256FromString(txHashStr)
				utxo := NewUTXO(txHash, uint32(i), amount, []byte{0x76, 0xa9})
				utxoSet.Add(utxo)
			}
...
// TestUTXO_ScriptPubKey tests UTXO ScriptPubKey getter
func TestUTXO_ScriptPubKey(t *testing.T) {
	txHash := Hash256{0x01, 0x02, 0x03}
	outputIndex := uint32(0)
	amount := uint64(1000000000)
	scriptPubKey := []byte{0x76, 0xa9, 0x14, 0xab, 0xcd, 0xef}

	utxo := NewUTXO(txHash, outputIndex, amount, scriptPubKey)

	result := utxo.ScriptPubKey()
	if len(result) != len(scriptPubKey) {
		t.Errorf("Expected script length %d, got %d", len(scriptPubKey), len(result))
	}

	for i, b := range scriptPubKey {
		if result[i] != b {
			t.Errorf("Expected script byte %d to be %x, got %x", i, b, result[i])
		}
	}
}
