...
func TestMerkleTree_Construction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var txHashes []bitcoin.Hash256
			for _, hexHash := range tt.txHashes {
				hash, err := bitcoin.NewHash256FromString(hexHash)
				if err != nil {
			...
			merkleRoot := bitcoin.CalculateMerkleRoot(txHashes)

			// Convert expected root to Hash256 for comparison
			expectedHash, err := bitcoin.NewHash256FromString(tt.expectedRoot)
			if err != nil {
...
func TestMerkleTree_GenesisBlock(t *testing.T) {
	...
	hash, err := bitcoin.NewHash256FromString(genesisTxHash)
	if err != nil {
		t.Fatalf("Failed to parse genesis tx hash: %v", err)
	}

	// Calculate merkle root (should equal the single transaction hash)
	merkleRoot := bitcoin.CalculateMerkleRoot([]bitcoin.Hash256{hash})
...
func TestMerkleTree_EmptyInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			merkleRoot := bitcoin.CalculateMerkleRoot(tt.txHashes)
